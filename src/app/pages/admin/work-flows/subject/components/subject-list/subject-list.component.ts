...
import {SubjectInterface} from '../../subject.state';
...
export class SubjectListComponent implements OnInit {
    ...
    private readonly allItems = this.subjectStore.items;
    ...
    private searching(): void {
        effect(() => {
            ...
            if (term) this.findSubject(1, term);
            else this.findSubject();
        });
    }
     ...
     */
    private findSubject(page = 1, search = '') {
        untracked(() => {
    ...
    private delete(item: SubjectInterface): void {
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.subjectStore.items.update(items =>
                    items.filter(current => current.id !== item.id)
                );
                this.findSubject(this.pagination().page, this.search());
            },
        });
    }
    ...
    protected onPageChange(paginatorState: PaginatorState) {
        ...
        // armar un bucle findSubject <-> paginator que congela la UI.
        const requestedPage = (paginatorState?.page ?? 0) + 1;
        const currentPage = this.pagination().page;

        if (paginatorState && requestedPage !== currentPage) {
            this.findSubject(requestedPage, this.search());
        }
    }
}
